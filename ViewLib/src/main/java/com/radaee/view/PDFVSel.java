...
public class PDFVSel
{
	...
	protected void SetSel( float x1, float y1, float x2, float y2 )
	{
		if( !m_ok )
		{
			m_page.ObjsStart();
	...
	protected void DrawSel( Canvas canvas, float scale, float page_height, int orgx, int orgy )
	{
		...
		m_paint.setColor(Global.selColor);
		m_page.ObjsGetCharRect(m_index1, rect);
		rect_word[0] = rect[0];
		rect_word[1] = rect[1];
		rect_word[2] = rect[2];
