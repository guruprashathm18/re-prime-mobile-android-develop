...
abstract class CalendarPagerView extends ViewGroup
  ...
  public void setSelectedDates(Collection<CalendarDay> dates) {
    Object[] list2 = dates.toArray();
    ArrayList<CalendarDay> calendarDays=new ArrayList<>() ;
    if(dates.size()>0){
      calendarDays.add((CalendarDay)list2[0]);
      if(dates.size()>1)
        calendarDays.add((CalendarDay)list2[dates.size()-1]);
    }
    for (DayView dayView : dayViews) {
      CalendarDay day = dayView.getDate();
      dayView.setChecked(calendarDays.contains(day));
      if (calendarDays.contains(day)){
        dayView.setTextColor(Color.parseColor("#0e0e0e"));
      }else {
